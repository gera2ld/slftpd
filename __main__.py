	...
	def ftp_RETR(self, args):
		...
		if os.path.isfile(realpath):
			yield from self.push_data(
					FileProducer(realpath,self.type,self.ret))
		else:
...
if __name__=='__main__':
	...
	conf=FTPHandler.conf=FTPConfig(loop)
	conf.add_user(FTPUser(
		perm='elrw',
		loginmsg='User ANONYMOUS okay, use email as password.',
	))
	conf.set_ports(8030,8040)
	coro=loop.create_server(FTPHandler,conf.host,conf.port)
